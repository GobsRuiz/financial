...
export function useAlerts() {
  ...
  const unpaidCreditByAccount = computed(() => {
    const grouped = new Map<number, number>()
    for (const tx of transactionsStore.transactions) {
      if (tx.paid || tx.payment_method !== 'credit') continue
      grouped.set(tx.accountId, (grouped.get(tx.accountId) ?? 0) + tx.amount_cents)
    }
    return grouped
  })
  ...
  const invoiceClosingAlerts = computed<AlertItem[]>(() => {
    ...
    for (const account of accountsStore.accounts) {
      if (!account.card_closing_day) continue
      const openAmountCents = Math.abs(unpaidCreditByAccount.value.get(account.id) ?? 0)

      const targetDate = dateForDayInCurrentMonth(today.value, account.card_closing_day)
      const daysUntil = dayjs(targetDate).startOf('day').diff(today.value, 'day')
